...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Article.objects.create(title=u'Верхнее меню', content=u'Верхнее меню', order=1, path='01')
        orm.Article.objects.create(title=u'Боковое меню', content=u'Боковое меню', order=1, path='02')

        orm.Article.objects.create(title=u'Список ролей', content=u'Список ролей', order=200, path='03', redirect='/roles')

        orm.Article.objects.create(title=u'Форма заявки', content=u'Форма заявки', order=300, path='04', redirect='/form')
        orm.Article.objects.create(title=u'Запись на питание', content=u'Запись на питание', order=1, path='0401', redirect='/food')
        orm.Article.objects.create(title=u'Запись на автобус', content=u'Запись на автобус', order=2, path='0402', redirect='/bus')
        orm.Article.objects.create(title=u'Поселение', content=u'Поселение', order=2, path='0403', redirect='/rooms')
